	...
	Function ConsultaCuentasBancariasDAL(deudor As Integer, identidad As Integer, model As List(Of ctaproveEntidad)) As Result
		...
		Try
			...
			Dim values = (From a In context.ctaprove
					  Join b In context.webbancos On a.cuentaid Equals b.cuentaid
					  Where a.identidad = identidad And a.deudor = deudor
					  Select a, b Select New ctaproveEntidad With {.id = a.id, .cuenta = a.cuenta, .moneda = a.moneda, .activo = a.activo, .nombre = b.shortname, .numrec = a.numrec, .clabe = a.clabe}).ToList()
