...
@Section Scripts
  @If TryCast(Session("Permisos"), List(Of Integer)).Contains(nuve.Permisos.Acciones.COBRANZA_ACTUALIZAR) Then
    @<script src="~/Scripts/CobranzaScripts/adeudos.js?v=@String.Concat(Today.Day, Today.Month, Today.Year, TimeOfDay.Hour, TimeOfDay.Minute, TimeOfDay.Second)"></script>
  Else
    @<script src="~/Scripts/CobranzaScripts/adeudosRonly.js?v=@String.Concat(Today.Day, Today.Month, Today.Year, TimeOfDay.Hour, TimeOfDay.Minute, TimeOfDay.Second)"></script>
  End If
End Section
