...
var data =
{
    inicializarTabla: function (contrato) {
        ...
        tabla = $('#table').DataTable({
                ajax: {
                    "url": "../Catalogos/ObtenerCesiones/", // url del controlador a consultar 
                    "Type": "GET",
           ...
           },
           {
               data: function (data, type, dataToSet) {
                   return '<a class="cancelar" href="../Catalogos/CesionWzd?contrato='+$("#contrato").val()+'&producto='  + $("#producto").val() + '&cesion=' + data.cesion + '&cliente=' + data.cliente + '&nombre=' + $("#clienteNombre").val() + '"><button type="button" class="btn glyphicon glyphicon-search"></button></a>';
                   //cesion=' + data.cesion + '&producto=' + $("#producto").val() + '&clienteNombre=' + $("#clienteNombre").val() + '&contrato='+$("#contrato").val()+'&last=true"><button type="button" class="btn glyphicon glyphicon-search"></button></a>';
               }, "width": "50px"
           }
                ],
                "columnDefs": [
        ...
        //.contrato = Model.contrato, .producto = Model.producto, .cesion = 0, .cliente = Model.cliente,.nombre = Model.clienteNombre
