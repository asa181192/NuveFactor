...
Public Class ValidacionesDAL
	...
	Function validarSirac(sirac As Integer, deudor As Integer, ByRef existe As Boolean, ByRef nombre As String) As Result
		...
		Using context As New FactorContext
			Try
				nombre = context.proveedor.Where(Function(e) e.deudor <> deudor And e.sirac = sirac And sirac <> 0).Select(Function(e) e.nombre).SingleOrDefault()
	...
	Function validarFira(fira As Decimal, deudor As Integer, ByRef existe As Boolean, ByRef nombre As String) As Result
		...
		Using context As New FactorContext
			Try
				nombre = context.proveedor.Where(Function(e) e.deudor <> deudor And e.fira_idcon = fira And e.fira_idcon <> 0).Select(Function(e) e.nombre).SingleOrDefault()
