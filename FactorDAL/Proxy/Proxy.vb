...
Public Class Proxy
    ...
    Public Function CancelaCFDI(rfc As String, serie As String, folio As Integer) As Result
        Dim response = New Result(False)
        Dim resuSer = New ServicioCancelacionCFDI.OutputOfCancelar()

        Try
            Dim consulta = New ServicioCancelacionCFDI.CancelacionServiceSoapClient()
            resuSer = consulta.CancelarCFDI(rfc, serie, folio)
            response.Ok = True
        Catch ex As Exception
            response.Mensaje = ex.Message
        End Try
