...
#Region "Consultas"
	...
	Function ConsultaComplementoDAL(tipo As Integer, fecha As Date, ByRef lista As List(Of FacturacionEntidad)) As Result
		Dim resultado = New Result(False)

		Using context As New FactorContext
			Try
				If tipo = 1 Then
					lista = (From A In context.pagosadeudos
						Join B In context.contratos
						On A.contrato Equals B.contrato
						Join C In context.clientes
						On C.cliente Equals B.cliente
						Where A.sat > 0 And A.fecha.Value.Month = fecha.Month And A.fecha.Value.Year = fecha.Year
						Select New FacturacionEntidad With {
							.fecha = A.fecha,
							.id = A.id,
				...
				Else
					lista = (From A In context.cfdicartera
					  Join B In context.contratos
					  On A.contrato Equals B.contrato
					  Join C In context.clientes
					  On C.cliente Equals B.cliente
				...
				If lista IsNot Nothing Then
					...
					For Each item As FacturacionEntidad In lista
						If item.identidad = 1 Then
							item.email = context.clientes.Where(Function(x) x.cliente = item.id).Select(Function(x) x.email).FirstOrDefault()
						ElseIf item.identidad = 2 Then
							item.email = context.proveedor.Where(Function(x) x.deudor = item.id).Select(Function(x) x.email).FirstOrDefault()
						Else
							item.email = context.comprador.Where(Function(x) x.deudor = item.id).Select(Function(x) x.email).FirstOrDefault()
						End If
					Next
	...
	Function enviarFacturaDAL(factura As String, identidad As Integer, id As Integer, numrec As Integer, tipo As Integer) As Result
		Dim response = New Result(False)
		Dim mail As String = Nothing
		Dim proxy = New Proxy()
		Dim archivoPdf As Byte() = Nothing
		Dim archivoXml As Byte() = Nothing
		Dim email = New sendMail()

		response = proxy.GetPdfResult(factura, archivoPdf)
		response = proxy.GetXmlResult(factura, archivoXml)

		Using context As New FactorContext
			Try
				If identidad = 1 Then
					mail = context.clientes.Where(Function(x) x.cliente = id).Select(Function(x) x.email).FirstOrDefault()
				ElseIf identidad = 2 Then
					mail = context.proveedor.Where(Function(x) x.deudor = id).Select(Function(x) x.email).FirstOrDefault()
				Else
					mail = context.comprador.Where(Function(x) x.deudor = id).Select(Function(x) x.email).FirstOrDefault()
				End If


				If archivoPdf IsNot Nothing And archivoXml IsNot Nothing And mail IsNot Nothing Then

					Dim listTo As String()
					Dim cc As String = ""
					Dim subject As String = "Complemento"
					Dim body As String = ""

					listTo = mail.Split(";")

					Dim attach = New Dictionary(Of String, Byte())

					attach.Add("complemento.pdf", archivoPdf)
					attach.Add("complemento.xml", archivoXml)

					body = "<p>Buen Día.<br>"
					body = body & "Se anexan la factura correspondiente<br><br>"
					...
					If tipo = 1 Then
						context.pagosadeudos.Where(Function(A) A.numrec = numrec).SingleOrDefault().envio = True
					Else
						context.cfdicartera.Where(Function(A) A.Idrec = numrec).SingleOrDefault().envio = True
					End If

					If email.enviarMail(listTo, subject, body, Net.Mail.MailPriority.Normal, Nothing, Nothing, Nothing, attach).Ok Then
						context.SaveChanges()
						response.Ok = True
					End If
...
#End Region
	...
	Function EnvioMasivoDal(fecha As Date, tipo As Integer) As Result
		Dim lista As List(Of FacturacionEntidad)
		Dim response = New Result(False)
		Dim archivoPdf As Byte() = Nothing
		Dim archivoXml As Byte() = Nothing
		Dim proxy = New Proxy()
			...
			Try
				If tipo = 1 Then
					lista = (From A In context.pagosadeudos
						...
						Join C In context.clientes
						On C.cliente Equals B.cliente
						Where A.sat > 0 And A.fecha.Value.Month = fecha.Month And A.fecha.Value.Year = fecha.Year And A.envio = 0
						Select New FacturacionEntidad With {
							.fecha = A.fecha,
							.id = A.id,
							.identidad = A.identidad,
				...
				For Each item As FacturacionEntidad In lista
					response = proxy.GetPdfResult("BVM951002LX0_" + item.serie + "_" + Convert.ToString(item.sat), archivoPdf)
					response = proxy.GetXmlResult("BVM951002LX0_" + item.serie + "_" + Convert.ToString(item.sat), archivoXml)

					If item.identidad = 1 Then
						mail = context.clientes.Where(Function(x) x.cliente = item.id).Select(Function(x) x.email).FirstOrDefault()
					ElseIf item.identidad = 2 Then
						mail = context.proveedor.Where(Function(x) x.deudor = item.id).Select(Function(x) x.email).FirstOrDefault()
					Else
						mail = context.comprador.Where(Function(x) x.deudor = item.id).Select(Function(x) x.email).FirstOrDefault()
					End If
					...
					If archivoPdf IsNot Nothing And archivoXml IsNot Nothing And mail IsNot Nothing Then
						...
						attach.Add("factura.pdf", archivoPdf)
						attach.Add("factura.xml", archivoXml)
						...
						If email.enviarMail(listTo, subject, body, Net.Mail.MailPriority.Normal, Nothing, Nothing, Nothing, attach).Ok Then
							If tipo = 1 Then
								context.pagosadeudos.Where(Function(A) A.numrec = item.numrec).SingleOrDefault().envio = True
							Else
								context.cfdicartera.Where(Function(A) A.Idrec = item.numrec).SingleOrDefault().envio = True
							End If
							context.SaveChanges()
