...
var Fact =
{
    inicializarTabla: function (serie, fecha) {
        ...
        tablaFinanciero = $('#table')
           ...
           {
               data: "idfiscalpf", "width": "40px", "render": function (data) {
                   return '<a class="cancel" id="' + data + '" style="cursor:pointer"><image src="../Images/cross.png"></button></a>'
                   //return '<a class="cancel" href="../Facturacion/GetFileFactura?factura=' + data + '&tipo=2" target="_self" ><image src="../Images/cross.png"></button></a>';
               }, orderable: false, className: "dt-body-center"
           },
           {
               data: "sat", "width": "40px", "render": function (data) {
                   return '<a class="replace" id="' + data + '" style="cursor:pointer"><image src="../Images/exchange.png"></button></a>'
               }, orderable: false, className: "dt-body-center dt-head-center"
           }
...
$(document).ready(function (e) {
    ...
    $('#tableContainer').on('click', 'a.replace', function (e) {
        
        sat = $(this).attr('id')

        cancelado = $(this).parents("tr").find("td")[10].innerHTML;

        $('#dialog-confirm').attr('title', '')


        if (sat <= 0 || cancelado == 'Cancelado') {

            message = (sat <= 0 ? 'El documento no cuenta con comprobante fiscal' : 'El documento ya está cancelado.')

            
            mensajemodal(message,'warning','Sustituir documento')
        } else {

            $('#mensaje').text('¿Desea reemplazar el documento?')

            divTitle = 'Sustitución de Documento'            
                    ...
                    buttons: {
                        "Sustituir documento": function () {
                            ...
                            $.ajax({
                                ...
                                url: "../Facturacion/SustituirFolioSat",                                
                                data: JSON.stringify(data),
                                success: function (e) {

                                    if (e.process != "") {
                                        mensajemodal(e.Mensaje, 'success', 'Sustitución de documento')
                                        Fact.inicializarTabla(ddlSerie, '01/' + fecha.val())
                                    }
                                    else {
                                        mensajemodal(e.Mensaje, 'warning', 'Sustitución de documento')
                                    }
                                },
                                error: function () {
                                    mensajemodal('No se logró obtener la información, intente nuevamente.', 'warning', 'Problemas de Conexión(Ajax)')
    ...
    $('#tableContainer').on('click', 'a.cancel', function (e) {
        ...
        cancelado = $(this).parents("tr").find("td")[10].innerHTML;
        sisfol = $(this).parents("tr").find("td")[6].innerHTML;

        divTitle = "Cancelación de Documentos."

        

        if (cancelado == 'Cancelado') {

            mensajemodal('La factura ya esta cancelada.', 'warning', 'Cancelación de factura')

        } else {
            $('#mensaje').text('¿Desea cancelar el documento?')
                    ...
                    buttons: {
                        "Cancelar Factura": function () {
                            ...
                            $.ajax({
                                ...
                                datatype: "JSON",
                                contentType: "application/json; charset=utf-8",
                                url: "../Facturacion/CancelarFactura",
                                //data: { 'serie': ddlSerie, 'sisfol': sisfol },
                                data: JSON.stringify(data),
                                success: function (e) {

                                    if (e.process == "1") {
                                        mensajemodal(e.Mensaje, 'success', 'Cancelación de Factura')
                                        Fact.inicializarTabla(ddlSerie, '01/' + fecha.val())
                                    }
                                    else {
                                        mensajemodal(e.Mensaje, 'warning', 'Cancelación de Factura')
                                    }
                                    $.Loading(false)
                                },
                                error: function () {
