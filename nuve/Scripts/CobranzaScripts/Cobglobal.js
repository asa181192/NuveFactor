...
var formulario =
{
  inicializarTabla: function (fecha, contrato) {
    tablaprincipal = $('#tablaprincipal')
     ...
     { data: "safectado", "width": "20px" },
     { data: "contrato", "width": "50px" },
     { data: "Nombre", "width": "500px" },
     { data: "monedastr", "width": "40px" },
     { data: "deposito", render: $.fn.dataTable.render.number(',', '.', '2', ''), orderable: false, className: 'dt-body-right', "width": "80px" },
    ...
    $("#test").change(function (w) {
      ...
      $.each(obj, function (k, v) {
        if ($("#test").is(":checked")) {
          ...
          if ($("#prueba").val() == 1) {
            $(table.rows(k).nodes()[0]).children()[8].textContent = $.fn.dataTable.render.number(',', '.', 2, '').display(parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')))
            $(table.rows(k).nodes()[0]).children()[7].textContent = $.fn.dataTable.render.number(',', '.', 2, '').display(0)

            //guardamos en array temporal
            gdata[k].neto = parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')).toFixed(2)
            gdata[k].descto = $(table.rows(k).nodes()[0]).children()[7].textContent

          } //Revisas opcion 2 de dropdown
          else if ($("#prueba").val() == 2) {
            $(table.rows(k).nodes()[0]).children()[7].textContent = $.fn.dataTable.render.number(',', '.', 2, '').display((parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')) *
               parseFloat($("#porc_aforo").val())
              ))


            $(table.rows(k).nodes()[0]).children()[8].textContent = $.fn.dataTable.render.number(',', '.', 2, '').display((parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')) -
              parseFloat($(table.rows(k).nodes()[0]).children()[7].textContent.replace(/,/g, ''))))

            gdata[k].descto = (parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')) *
                 parseFloat($("#porc_aforo").val())
                ).toFixed(2)

            gdata[k].neto = (parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')) -
            ...
            $(table.rows(k).nodes()[0]).children()[7].textContent = $.fn.dataTable.render.number(',', '.', 2, '').display(parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')))
            $(table.rows(k).nodes()[0]).children()[8].textContent = $.fn.dataTable.render.number(',', '.', 2, '').display(0)

            gdata[k].descto = parseFloat($(table.rows(k).nodes()[0]).children()[6].textContent.replace(/,/g, '')).toFixed(2)
            gdata[k].neto = $(table.rows(k).nodes()[0]).children()[8].textContent
          }
...
$(document).ready(function (e) {
  ...
  $("#sfecha").datepicker({
    dateFormat: 'dd/mm/yy', language: 'es', locale: 'es',
    onSelect: function (e, obj) {
      formulario.inicializarTabla(e, fecha.val());
    }
  }).datepicker("setDate", new Date())
  formulario.inicializarTabla(fecha.val()); // Realiza consulta al mes actual y año actual al carga la pagina pro primera vez 
  ...
  btnafectar.click(function (e) {
    ...
    var teso = $("#neto").text().replace(a, '')
    var cob = $("#deposito").text().replace(a, '')
    var afec= $("#afectado").text()
    ...
    if (afec == "0") {
      ...
      if (teso !== cob) {
        mensajemodal('Cobranza presenta diferencias, no puede ser afectada.', 'Warning')
      }
      else {

        $('#dialog-confirm').text("¿Desea afectar cobranza?")
        var url = "../cobranza/Afectacobglobal/?fecha=" + fecha.val();
